...
namespace Logic.UI
{
    public partial class ChatbotMain : Control
    {
        ...
        {
            // Nota: Este método se mantiene como mock para pruebas de media
            // pero podrías refactorizarlo luego para usar también los componentes.
            OnBotStartedThinking();
            await ToSignal(GetTree().CreateTimer(3.0f), SceneTreeTimer.SignalName.Timeout);

            if (_mensajeBotActual != null) _mensajeBotActual.FinalizarRespuesta();

            // Lógica de inserción de media manual (se mantiene igual por ahora)
            // ... (Código de VideoStreamPlayer / TextureRect omitido para brevedad pero mantenido en tu archivo)
        ...
        {
            // BYPASS TOTAL AL AUTOLOAD: Cargamos el archivo directo del disco
            string path = isDark ? "res://Resources/UI_Themes/TemaOscuro.theme" : "res://Resources/UI_Themes/TemaClaro.theme";
            Theme temaCorrecto = ResourceLoader.Load<Theme>(path);

            if (MessagesContainer != null)
            {
                MessagesContainer.Theme = temaCorrecto;

                // Obligamos a los mensajes que ya están en pantalla a recibir el tema
